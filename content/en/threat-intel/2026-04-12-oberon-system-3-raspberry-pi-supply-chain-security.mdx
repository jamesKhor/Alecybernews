...
For environments where Raspberry Pis are deployed, monitoring for the use of unauthorized operating systems is key. No specific signatures for a compromised Oberon image exist, but behavioral detection is possible.
*   **Network Detection:** Hunt for HTTP/HTTPS requests from Raspberry Pi hardware MAC addresses (OUI `DC:A6:32`, `B8:27:EB`, `E4:5F:01`) to unexpected domains or IPs, especially if the host is not running a standard Linux distribution.
*   **Endpoint/Host Detection:** On the SD card itself, analysts could attempt to verify the integrity of the `OberonSystem3Native.img` file against a cryptographically signed hash provided by a trusted source—though no such trusted source currently exists for this artifact.
*   **Process Analysis:** This is not applicable for monitoring from a host-based perspective on the Pi running Oberon, as any monitoring agent would likely be part of the compromised stack.
