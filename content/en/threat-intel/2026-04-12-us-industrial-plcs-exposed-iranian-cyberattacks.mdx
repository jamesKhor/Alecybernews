...
Organizations, especially in critical infrastructure, should hunt for the following activity:
*   Network traffic logs showing connection attempts to OT assets (particularly Rockwell PLCs on TCP/44818) from unexpected external IP addresses, especially those geolocated to regions associated with Iranian infrastructure.
*   **Zeek/Suricata Signature:** Alert for `ET SCAN Rockwell EtherNet/IP Scanner` or similar signatures detecting scans for port 44818.
*   **Internal Network Hunting:** Look for connections to PLCs from engineering workstations or servers that do not normally communicate with them, indicating potential lateral movement.
*   **Firewall/Proxy Logs:** Query for outbound connections from the OT network to known threat intelligence feeds or unfamiliar external IPs, which may indicate beaconing from a compromised HMI.
