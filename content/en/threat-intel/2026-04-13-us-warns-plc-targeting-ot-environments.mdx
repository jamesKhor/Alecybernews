...
Organizations should monitor for unusual traffic patterns between IT and OT network segments, particularly involving legacy protocols:
```sql
SELECT * FROM network_traffic WHERE protocol IN ('Modbus', 'DNP3') AND src_zone = 'IT' AND dst_zone = 'OT'
```
Additionally, query logs for repeated authentication failures against known PLC IP addresses:
```sql
SELECT count(*) AS login_attempts, src_ip FROM auth_logs WHERE device_type LIKE '%PLC%' GROUP BY src_ip HAVING login_attempts > 10
```
Anomalous binary changes or unexpected logic uploads to controllers should also raise suspicion.
