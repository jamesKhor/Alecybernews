...
Organizations should monitor their SIEM and EDR platforms for anomalous behavior indicative of OT-targeted intrusion:

- Unusual outbound connections from OT hosts to external IPs
- Logins during off-hours to engineering stations or HMIs
- Unexpected changes in PLC program files (`*.awl`, `*.sdb`) or configuration dumps
- Anomalous use of industrial protocols outside normal baselines (Modbus TCP port 502, DNP3 port 20000)
- High-frequency read/write requests to PLC registers from non-standard sources

Sample Splunk query:
```
tag=modbus earliest=-7d@d | stats count by src_ip, dest_ip | where count > 1000
```

YARA rules scanning for suspicious binary modifications in known ICS software directories can also aid in detection.
