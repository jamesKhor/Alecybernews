...
Organizations can search logs for anomalies indicating potential AiTM activity:
- Unusual geolocation shifts during authentication flows
- Unexpected increases in outbound traffic from internal IP addresses to known suspicious domains
- Repeated failed modifications followed by one successful change to payroll records

Example Splunk query:
```
tag=authentication action=success earliest=-7d@d 
| iplocation src_ip 
| stats count by Country, user, _raw
```

SIEM rules should also flag repeated POST requests to `/login` endpoints originating from single IPs but associated with multiple usernames over short intervals.
