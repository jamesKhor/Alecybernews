...
- **Windows Sysmon**: `EventID=1 AND ImageLoaded LIKE "%extension.js%"`
- **Endpoint logs**: `process_name: "code.exe" AND command_line:*"specstudio.code-wakatime-activity-tracker*"`
- **Network**: `dst.domain: "dlz9x.cloudfront.net" AND dst.port:443`
- **File integrity**: Alert on newly created hidden files under `%USERPROFILE%\.vscode\` or JetBrains config directories.
