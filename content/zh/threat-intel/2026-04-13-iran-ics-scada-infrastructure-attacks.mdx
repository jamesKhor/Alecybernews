...
## 检测与溯源查询
各机构应密切监视SIEM和EDR平台，查找可能指示OT定向入侵的异常行为：

- 来自OT主机对外部IP地址的异常外联请求  
- 在非工作时间登录工程站或HMI系统的记录  
- PLC程序文件（如`*.awl`, `*.sdb`）或配置转储出现意外更改  
- 工业协议在正常基线之外的异常使用情况（如Modbus TCP端口502，DNP3端口20000）  
- 非标准来源频繁读写PLC寄存器的操作  
...
此外，在已知ICS软件目录中扫描可疑二进制文件修改的YARA规则也可辅助检测。
