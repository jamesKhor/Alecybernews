...
组织应优先修补。对于无法立即修补的环境中的检测，可以考虑以下狩猎策略：
*   监控网络日志，查找针对 SRX 和 EX 系列设备 IP 上 `/juniper/` 路径或其他 J-Web 特定 URI 的 HTTP 请求，特别是来自意外的源 IP 或地理位置。
*   部署专注于针对 Juniper 网络界面的异常 HTTP 数据包大小或结构的入侵检测签名，尽管这些可能被复杂的攻击规避。
*   使用 FreeBSD 主机上的端点检测来查找 J-Web 服务生成的意外子进程或系统文件的未经授权的更改。
*   查询失败的登录尝试，紧接着是成功的利用尝试，尽管这个漏洞不需要身份验证。
